...
func main() {
	//参加百米运动会，如果用时 8 秒以内进入决赛，
	// 否则提示淘汰。并且根据性别提示进入男子组或女 子组。【可以让学员先练习下】,
	// 输入成绩和性别。
	...
	*/
	...
	fmt.Println("请输入游玩月份")
	fmt.Scanln(&month)
	fmt.Println("请输入游客年龄的")
	fmt.Scanln(&age)

	if month >= 4 && month <= 10 {
		if age > 60 {
			fmt.Printf("%v月 年龄 %v 票价：%v", month, age, price/3)
		} else if age >= 18 {
		...
		//淡季
		if age >= 18 && age < 60 {
			fmt.Printf("淡季成人 票价 40")
		} else {
			fmt.Printf("淡季儿童和老人 票价 20")
		}
	}
}
